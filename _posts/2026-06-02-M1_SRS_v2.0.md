...
| June 2, 2026 | Yiding Wang | Integrated doctor binding and schedule enhancement requirements; expanded UC-M1-01-01 and IUC-M1-01-01; added UC-M1-04-02, UC-M1-04-03, UC-M1-06-01 and corresponding internal use cases |
| June 9, 2026 | Diogo Pinhel | Architecture conformity review against validated HLD V2: corrected IUC-M1-02-02 Step 7 and IUC-M1-02-03 to reflect M1-mediated asynchronous upload and polling model; S2 does not communicate with V2 directly — M1 reads from S2 via readLoop (10 Hz) and uploads via uploadLoop (10 Hz); real-time feedback uses async polling (15 s) not synchronous request-response |
...
| 7 | | Reads data from S2 at 10 Hz (readLoop), uploads batches to V2 via `POST /measurements/batch` (uploadLoop 10 Hz), and maintains a local in-memory cache. S2 does not communicate with V2 directly. | | |
...
| **Summary** | M1 reads formal sensor data from S2 via a 10 Hz pull loop (readLoop), uploads measurement batches to V2 asynchronously via a separate 10 Hz upload loop (uploadLoop), and polls V2 for AI recommendations every 15 s (downloadLoop); M1 renders the latest joint angle and recommendations to the patient in real time. |
...
| Step | Patient | M1 | S2 | V2 |
| :--- | :--- | :--- | :--- | :--- |
| 1 | | readLoop drains buffered FormatData from S2 at 10 Hz. | | |
| 2 | | | Returns formal sensor data (FormatData: angles, raw IMU, errors) to M1. | |
| 3 | | uploadLoop dequeues FormatData and sends `POST /measurements/batch` to V2 at 10 Hz (async, independent of readLoop). | | |
| 4 | | downloadLoop polls `GET /recommendations/session/:id` from V2 every 15 s. | | |
| 5 | | | | Returns AI recommendations to M1. |
| 6 | | Renders latest joint angle (LegView) and AI recommendations to the patient. | | |
| 7 | Receives the feedback. | | | |

**Alternative Flow**

| Occurrence Step | Condition | System Response |
| :--- | :--- | :--- |
| 2 | S2 buffer is empty (no samples collected yet). | M1 skips this readLoop cycle silently; feedback is not updated until data is available. |
| 3 | uploadLoop upload fails (network error). | M1 re-queues the batch at the head of uploadQueue; retries on the next uploadLoop cycle without interrupting readLoop or downloadLoop. |
| 5 | V2 fails to return AI recommendations. | M1 retains the previous recommendations and retries on the next downloadLoop cycle (15 s). No error is shown to the patient for transient failures. |
...
| **Summary** | M1 ends exercise on S2 and V2; M1 builds the session summary locally from S2 session data (sample count, error count, start/end time, exercise type); M1 then fetches AI recommendations from V2 to display alongside the summary. |
| **Trigger** | Patient clicks "End Exercise". |
| **Precondition** | Patient has started a rehabilitation exercise. |
...
| 4 | | | Stops processing data and returns session metadata (sample count, error count, start/end time) to M1. | |
| 5 | | Sends a request to V2 to end the rehabilitation exercise. | | |
| 6 | | | | Closes the exercise record. |
| 7 | | Builds the session summary locally from S2 session metadata and current exercise type. | | |
| 8 | | Fetches AI recommendations from V2 (`GET /recommendations/session/:id`). | | |
| 9 | | | | Returns AI recommendations to M1. |
| 10 | | Displays the locally-built session summary and AI recommendations to the patient. | | |
| 11 | Views the summary. | | | |

**Alternative Flow**

| Occurrence Step | Condition | System Response |
| :--- | :--- | :--- |
| 8 | V2 fails to return AI recommendations (network or server error). | M1 displays the session summary without AI recommendations and informs the patient that evaluations may be viewed later in the rehabilitation history. |
...
| A-M1-02 | Target devices are Android smartphones (API 31+). The initial HarmonyOS target was revised mid-project due to hardware and toolchain constraints. |
...
- **NFR-M1-19**: The app must be built with Kotlin (Android) following M1 architectural conventions, chosen for its native BLE sensor integration capabilities.
